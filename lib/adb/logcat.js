      ...
      switch(state) {
        case kStateParseHeader:
          if (!available || available < 20) {
            ...
            switch (__pad) {
              case 0:   // struct logger_entry
                break;
              case 24:  // struct logger_entry_v2 and logger_entry_v3
                // This 4 bytes are for euid or logid. Just consume it.
                readArrayBuffer(4);
                break;
              default:
                assert.ok(false, `Parse Error : Unhandled version of adb log header version is detected : ${__pad}`);
            }
